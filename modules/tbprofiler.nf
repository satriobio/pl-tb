process fastqc {
  ...
  publishDir params.versioned_outdir ? "${params.outdir}/${sample_id}/${params.pipeline_short_name}-v${params.pipeline_minor_version}-output" : "${params.outdir}", pattern: "*fastqc*.html", mode: 'copy'

  input:
  tuple val(sample_id), path(reads_1), path(reads_2)

  output:
  tuple val(sample_id), path("${sample_id}_fastqc_R1.html"), path("${sample_id}_fastqc_R2.html"), emit: read
  ...
  f=${reads_1}
  g=${reads_2}
  mv \${f/.fastq.gz/}_fastqc.html ${sample_id}_fastqc_R1.html
  mv \${g/.fastq.gz/}_fastqc.html ${sample_id}_fastqc_R2.html
  """
}
...
process fastp {
  ...
  publishDir params.versioned_outdir ? "${params.outdir}/${sample_id}/${params.pipeline_short_name}-v${params.pipeline_minor_version}-output" : "${params.outdir}", pattern: "${sample_id}_fastp.{json,csv}", mode: 'copy'
...
process kraken2 {
    ...
    publishDir params.versioned_outdir ? "${params.outdir}/${sample_id}/${params.pipeline_short_name}-v${params.pipeline_minor_version}-output" : "${params.outdir}", mode: 'copy', pattern: "${sample_id}_kraken.output"
    publishDir params.versioned_outdir ? "${params.outdir}/${sample_id}/${params.pipeline_short_name}-v${params.pipeline_minor_version}-output" : "${params.outdir}", mode: 'copy', pattern: "${sample_id}_kraken*.txt"
    ...
    output:
    tuple val(sample_id), path("${sample_id}_kraken.output"), emit: kraken
    
    script:
    """
    kraken2 -db ${kraken2_db} --report ${sample_id}_kraken_report.txt -paired -1 ${reads_1} -2 ${reads_2} > ${sample_id}_kraken.output
    """
}
...
process krona {
    ...
    publishDir params.versioned_outdir ? "${params.outdir}/${sample_id}/${params.pipeline_short_name}-v${params.pipeline_minor_version}-output" : "${params.outdir}", mode: 'copy', pattern: "${sample_id}_krona.html"
    ...
    output:
    tuple val(sample_id), path("${sample_id}_krona.html") 
    tuple val(sample_id), path("${sample_id}_krona.output"), emit: krona

    script:
    """
    cat ${kraken} | cut -f 2,3 > ${sample_id}_krona.output
    ktImportTaxonomy -tax ${krona_db} ${sample_id}_krona.output
    mv *.html ${sample_id}_krona.html
    """
}
...
process tbprofiler {
    ...
    publishDir params.versioned_outdir ? "${params.outdir}/${sample_id}/${params.pipeline_short_name}-v${params.pipeline_minor_version}-output" : "${params.outdir}", mode: 'copy', pattern: "${sample_id}_tbprofiler*.{csv,json}"
    publishDir params.versioned_outdir ? "${params.outdir}/${sample_id}/${params.pipeline_short_name}-v${params.pipeline_minor_version}-output" : "${params.outdir}", mode: 'copy', pattern: "${sample_id}_tbprofiler*.{bam,bam.bai,vcf}", enabled: !params.rename_ref
...
process rename_ref_in_alignment {
  ...
  publishDir params.versioned_outdir ? "${params.outdir}/${sample_id}/${params.pipeline_short_name}-v${params.pipeline_minor_version}-output" : "${params.outdir}", mode: 'copy', pattern: "${sample_id}_tbprofiler*.{bam,bam.bai}", enabled: params.rename_ref, saveAs: { x -> x.replace("_renamed", "") }
...
process rename_ref_in_variants {
  ...
  publishDir params.versioned_outdir ? "${params.outdir}/${sample_id}/${params.pipeline_short_name}-v${params.pipeline_minor_version}-output" : "${params.outdir}", mode: 'copy', pattern: "${sample_id}_tbprofiler*.vcf", enabled: params.rename_ref, saveAs: { x -> x.replace("_renamed", "") }
...
process qualimap_bamqc {
    ...
    publishDir params.versioned_outdir ? "${params.outdir}/${sample_id}/${params.pipeline_short_name}-v${params.pipeline_minor_version}-output" : "${params.outdir}", mode: 'copy', pattern: "${sample_id}_qualimap_alignment_qc.csv"
